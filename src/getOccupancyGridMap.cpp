...
void readMap(const nav_msgs::OccupancyGrid& map, ros::NodeHandle &nh){
    ...
    double ziel_map_frame_x = grid_cell_x[50] * map.info.resolution + map.info.origin.position.x;
    double ziel_map_frame_y = grid_cell_y[100] * map.info.resolution + map.info.origin.position.y;
    double mark_pos_x = ziel_map_frame_x;
    double mark_pos_y = ziel_map_frame_y;
    int id = 0;
    setMarker(nh, mark_pos_x, mark_pos_y, id);
